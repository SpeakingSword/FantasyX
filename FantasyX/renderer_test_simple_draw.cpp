...
#pragma endregion
    ...
    renderer->windowWith = SCR_WIDTH;
    renderer->windowHeight = SCR_HEIGHT;
    ...
    base_simple_mat->metallic = 1.0f;
    base_simple_mat->roughness = 0.2f;
    base->SetMaterial(base_simple_mat);

    GameObject *dirLight = GameObject::DirLight();
    dirLight->transform->rotation.x = 0.0f;
    dirLight->transform->rotation.y = 90.0f;
...
#pragma region 绘制GUI
...

        {
            ...
            ImGui::SliderFloat("ModelPosY", &model->transform->position.y, -3.0f, 3.0f);
